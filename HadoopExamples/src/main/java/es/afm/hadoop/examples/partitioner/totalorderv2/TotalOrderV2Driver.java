...
public class TotalOrderV2Driver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		if (args.length != 2) {
			System.err
					.println("TotalOrderV2 required params: <input_path> <output_path>");
			System.exit(-1);
		}
		...
		RandomSampler<LongWritable, Text> sampler = new InputSampler.RandomSampler<LongWritable, Text>(
				0.1, 100);
		TotalOrderPartitioner.setPartitionFile(job.getConfiguration(),
				new Path(args[0] + "/_partition.lst"));
